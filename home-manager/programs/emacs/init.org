  ...
  (use-package project
    ...
    (defun project-find-file-in (filename dirs project)
      ...
      (let* ((pr (project-root project))
             (remote (file-remote-p pr)))
        (find-file
         (selectrum--read
          (format "%s: " (file-name-base (directory-file-name (file-local-name pr))))
          (lambda (q)
            (mapcar (lambda (p) (concat remote p))
                    (split-string
                     (shell-command-to-string
                      (format "fd --color=never --max-results=%d \"%s\" %s" selectrum-num-candidates-displayed q (file-local-name pr)))
                     "\n" t)))))))
