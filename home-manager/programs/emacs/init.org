             ...
             (selection (selectrum--read
                         ...
                         (lambda (q)
                           (let* ((tokens (split-string q " " t))
                             ...
                             (split-string
                              (shell-command-to-string
                               (format "rg --vimgrep --color=never '%s' | head -n %d" pattern selectrum-num-candidates-displayed))
                              "\n" t)))
...
#+begin_src emacs-lisp
  ...
  (defun nuance/rg-xref--find-regexp (pattern)
    ...
    (let* ((project (project-current))
           ...
           (matches (split-string
                     (shell-command-to-string (format "rg --vimgrep --color=never '%s' | head -n %d" pattern (* 5 selectrum-num-candidates-displayed))) "\n" t)))
      (mapcar
       (lambda (m)
  ...
  (defun nuance/rg-xref-definition-regexp (pattern)
    (format "(((def|class)\\s*%s[(:])|(\\s+%s[^a-zA-Z0-9].*\\s+(in|=)))" pattern pattern))
...
#+begin_src emacs-lisp
  (use-package deferred  :defer 2)
  (use-package button-lock  :defer 2)
#+end_src
