...
#+begin_src emacs-lisp :tangle "early-init.el"
  ...
  (add-hook 'after-make-frame-functions (lambda (frame) (set-frame-parameter frame 'fullscreen-restore 'fullscreen)))
...
Show isearch hit information in modeline.
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package modus-themes
    :custom (modus-themes-org-blocks 'gray-background)
    :config
...
#+begin_src emacs-lisp
  (use-package embark
    ...
    ("<mouse-3>" . (lambda (event) (interactive "e") (mouse-set-point event) (embark-act))))
...
I've used mini-popup, mini-frame, and vertico-posframe, all of which I've found a little buggy. =mini-popup= seems to be the easiest to debug when things go wrong, so I'm sticking to that.
...
#+begin_src emacs-lisp
  (use-package mini-popup
    ...
    :when (window-system)
    :config
    ;; resize to at most 60% of the frame height
    (defun mini-popup-height-resize ()
      (min (floor (* 0.6 (frame-pixel-height))) (* (1+ (min vertico--total vertico-count)) (default-line-height))))
    ...
    (advice-add #'mini-popup--hide :around (lambda (&rest args) (when (and mini-popup-mode mini-popup--frame) (delete-frame mini-popup--frame))))

    ;; place the frame near the top
    (setf (alist-get 'top mini-popup--frame-parameters) 0.05
          (alist-get 'width mini-popup--frame-parameters) 0.9)

    (mini-popup-mode)

    ;; Ensure that the popup is updated after refresh (Consult-specific)
    :hook (consult--completion-refresh . (lambda (&rest _) (mini-popup--setup))))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package multiple-cursors
    ...
    (add-to-list 'mc/cmds-to-run-once 'nuance/mark-next-like-this-symbol)
...
#+begin_src emacs-lisp
  (global-set-key (kbd "s-k") (lambda () (interactive) (switch-to-buffer "*scratch*")))
#+end_src
...
#+begin_src emacs-lisp
  (use-package project
    ...
    :config
    (cl-defmethod nuance/project-find-by-name-cmd ((project t) q)
    ...
    (cl-defmethod nuance/project-find-by-name-initial ((project t)) "")
    ...
    (defun nuance/find-by-name-builder (input)
      "Build command line given INPUT."
      (pcase-let* ((cmd (nuance/project-find-by-name-cmd (project-current) input))
    ...
    See `consult-grep' for more details regarding the asynchronous search."
      ...
      (let* ((default-directory (project-root project))
             (path (nuance/find-by-name (format "%s: " (nuance/project-name project)) #'nuance/find-by-name-builder (nuance/project-find-by-name-initial project))))
...
#+begin_src emacs-lisp
  (use-package project
    ...
    :after (consult)
    :config
    (cl-defmethod nuance/find-by-content-dispatch ((project t))
      (consult-git-grep))
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "s-n") 'nuance/nv-find-note)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package eldoc-box :if (window-system) :hook ((prog-mode org-mode) . eldoc-box-hover-at-point-mode))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package cc-mode
    ...
    :mode ("\\.h|\\.cpp" . c++-mode)
    :hook (c++-mode . nuance/eglot-format-buffer-on-save)
