...
#+BEGIN_SRC emacs-lisp
  (use-package tramp
    ...
    (setq
     ...
     tramp-inline-compress-start-size 262144))
#+END_SRC
...
** Formatter
 ...
 #+BEGIN_SRC emacs-lisp
   ...
   (defun nuance/format-buffer (tmp-buf-name binary &optional failure-p &rest args)
     (if (> (buffer-size) tramp-inline-compress-start-size) (message "Skipping formatting for large file")
...
*** autobuildify
 ...
 #+BEGIN_SRC emacs-lisp
   ...
   (defun autobuildify--run-buildifier-on-current-file ()
     (interactive)
     (nuance/format-buffer "buildifier" autobuildify--buildifier-binary))

   (defun autobuildify--save-hook ()
     (let ((fname (buffer-file-name)))
       (if (and fname
                (string-match "/\\(TARGETS\\|BUILD\\)$" fname))
           (autobuildify--run-buildifier-on-current-file))))
